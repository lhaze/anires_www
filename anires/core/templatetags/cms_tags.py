...
from anires.core.models import Advert
...
def blog_listing_homepage(context, count=2):
    from anires.news.models import EntryPage
    blogs = EntryPage.objects.live().order_by('-date')
    return {
        'entries': blogs[:count].select_related('feed_image'),
        # required by the pageurl tag that we want to use within this template
        'request': context['request'],
    }
